...
import { Text, Row, Button, Modal, Input } from "@nextui-org/react";
...
export const GetCertificate = () => {
  ...
  const createCertificate = async () => {
    console.log(name, maticAmount);
    const fullName = name;
    const amount = maticAmount;
    const tokenUri = "we";
    if (!(amount > 0)) {
      setIsLoading(false);
    ...
    try {
      const tx = await contract.lockFunds(finalAmount, tokenUri, fullName, {
        value: finalAmount,
      });
      await tx.wait();
      setIsLoading(false);
